...
#         super().__init__()
#
#         self.norm1_img = nn.LayerNorm(inner_dim)
#         self.cross_attn_img = nn.MultiheadAttention(
#             embed_dim=inner_dim, num_heads=num_heads, kdim=image_feat_dim, vdim=image_feat_dim,
#             dropout=attn_drop, bias=attn_bias, batch_first=True)
#
#         self.norm1_text = nn.LayerNorm(inner_dim)
#         self.cross_attn_text = nn.MultiheadAttention(
#             embed_dim=inner_dim, num_heads=num_heads, kdim=text_feat_dim, vdim=text_feat_dim,
#             dropout=attn_drop, bias=attn_bias, batch_first=True)
#
#         self.norm2 = nn.LayerNorm(inner_dim)
#         self.self_attn = nn.MultiheadAttention(
...
#         )
#
#     def forward(self, x, image_feats, text_feats):
#         x = x + self.cross_attn_img(self.norm1_img(x), image_feats, image_feats)[0]
#         x = x + self.cross_attn_text(self.norm1_text(x), text_feats, text_feats)[0]
#         before_sa = self.norm2(x)
#         x = x + self.self_attn(before_sa, before_sa, before_sa)[0]
#         x = x + self.mlp(self.norm3(x))
#         return x
...
#         self.layers = nn.ModuleList([
#             BasicTransformerBlock(
#                 inner_dim=inner_dim, image_feat_dim=image_feat_dim, text_feat_dim=text_feat_dim, num_heads=num_heads,
#                 eps=eps)
#             for _ in range(num_layers)
#         ])
#         self.norm = nn.LayerNorm(inner_dim, eps=eps)
...
#         for layer in self.layers:
#             x = layer(x, image_feats, text_feats)
#         x = self.norm(x)
#
#         x = x.view(N, 3, H, W, -1)
